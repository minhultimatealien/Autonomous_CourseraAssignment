...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            if dt > 0:
                v_error_integral = v_error_pre_integral + v_error_cur * dt
                v_error_integral = max(min(v_error_integral, 10.0), -10.0)  #Anti-windup
            ...
            if(a_des + feedforward > 0.05) :
                ...
                elif (throttle_des - throttle_pre < (-0.1)):
                    throttle_des = throttle_pre - 0.1
                else:
                    pass
            elif(a_des + feedforward < -0.05):
                throttle_des = 0
                brake_output = max(0.0, min(1.0, -(a_des + feedforward)))
            else: 
                throttle_des = 0
                brake_output = 0

            throttle_des = max(0.0, min(1.0, throttle_des))
...
#            look_ahead = waypoints[look_ahead_index]
            ...
            look_ahead_lat_idx = min(self._closest_idx + 10, len(waypoints) - 1)
            look_ahead_pt = waypoints[look_ahead_lat_idx]
...
#            heading_error = yaw - np.arctan2(waypoints[1][1] - waypoints[0][1], waypoints[1][0] - waypoints[0][0])
            curr_wp = waypoints[self._closest_idx]
            next_wp = waypoints[min(self._closest_idx + 1, len(waypoints) - 1)]
            path_yaw = np.arctan2(next_wp[1] - curr_wp[1], next_wp[0] - curr_wp[0])
            ...
            if dt > 0.0:
                ...
                heading_error_derivative = 0.0

            feedback_lateral = kp_lat * heading_error + ki_lat * self.vars.heading_error_integral + kd_lat * heading_error_derivative


            ld = max(np.sqrt((look_ahead_pt[0] - x)**2 + (look_ahead_pt[1] - y)**2), 1.0)

            vector_look_ahead = [look_ahead_pt[0] - x, look_ahead_pt[1] - y]
